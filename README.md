...
- **Custom Pylint Checker**  
  Deep-scans `__all__` declarations using `astroid` to find duplicate exports across the package tree.

- **Language Server Protocol (LSP)**  
  Real-time feedback inside VS Code via a dedicated Python server (`pygls`).

- **Intelligent Filtering**  
  Recognizes valid patterns (e.g., Sync vs. Async clients) while flagging accidental duplicates.

- **CI/CD Ready**  
  Can be integrated into GitHub Actions to block PRs that violate namespace rules.
...
- **Core (Python)**  
  A custom Pylint plugin that performs AST (Abstract Syntax Tree) analysis.

- **Server (LSP)**  
  A Python-based Language Server that wraps the checker for editor communication.

- **Client (TypeScript)**  
  A VS Code extension that handles the UI and diagnostics display.
...
3. Open any Python file — duplicate exports in `__init__.py` will be highlighted automatically
...
* **Youssef Nabil**
  Software Engineer & Microsoft Ambassador

* GitHub: [https://github.com/youssefnabil2030/](https://github.com/youssefnabil2030/)

* LinkedIn: [https://www.linkedin.com/in/youssef-nabil-711558204/](https://www.linkedin.com/in/youssef-nabil-711558204/)
